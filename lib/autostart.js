...
/**
 * @hiker8668/dsh-feishu/autostart — a tiny launcher plugin that spawns the Feishu
 ...
 * @module @hiker8668/dsh-feishu/autostart
 */
...
/** The profile this process is running under, from the launcher args. */
...
function apply(ctx, config) {
	// Guard against recursion: if this process is already the target profile
	// (e.g. the feishu profile mounts this bundle), it IS the bot — don't spawn
	// a duplicate. Only a host profile (web, etc.) should spawn the bot.
	if (currentProfile() === config.profile) {
		ctx.logger.info(`[feishu-autostart] already running profile ${config.profile}, skip spawning`);
		return;
	}
