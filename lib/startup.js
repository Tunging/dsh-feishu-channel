...
/** Parse a JSON array of per-group rules; returns [] on any invalid input. */
function parseGroupRules(raw) {
	if (!raw) return [];
	try {
		const list = JSON.parse(raw);
		if (!Array.isArray(list)) return [];
		return list.filter((r) => r && typeof r.chatId === "string");
	} catch {
		return [];
	}
}
...
function buildConfig(opts, creds) {
	...
	return {
		...
		admins: parseList(opts.admins).join(","),
		workspaceManage: opts.workspaceManage === "all" ? "all" : "admin",
		groupRules: parseGroupRules(opts.groupRules),
		allowedTools: parseList(opts.allowedTools),
		dangerousCommands: parseList(opts.dangerousCommands),
		...
		enableMedia: opts.media !== false,
		projectContext: opts.projectContext ?? "",
		codeAgentModel: opts.codeAgentModel ?? "",
		codeAgentContext: opts.codeAgentContext ?? "",
		idleTimeoutMs: Number.parseInt(opts.idleTimeout, 10) || 0,
		idleCheckIntervalMs: Number.parseInt(opts.idleCheckInterval, 10) || 300000,
		replyTimeoutMs: Number.parseInt(opts.replyTimeout, 10) || 0,
		maxReplyChars: Number.parseInt(opts.maxReplyChars, 10) || 0,
		errorNotify: opts.errorNotify !== false,
		subscribeEvents: Boolean(opts.subscribeEvents),
...
function feishuCommand(ctx) {
	const program = new Command().name("dsh --profile feishu").description("Run a Feishu bot surface that drives a DeepSeek Harness agent.").helpOption("-h, --help", "show this help").option("--app-id <id>", "Feishu app id (or run `setup` to create one by scanning)").option("--app-secret <secret>", "Feishu app secret").option("--mode <mode>", "transport: 'webhook' (event subscription) or 'longconn' (WebSocket)", "webhook").option("--port <port>", "listen port for webhook mode", "8080").option("--host <host>", "webhook bind address", "0.0.0.0").option("--path <path>", "webhook endpoint path", "/").option("--domain <domain>", "'feishu' (China) or 'lark' (International)", "feishu").option("--workspace <dir>", "workspace root for fresh sessions", process.cwd()).option("--state-file <file>", "JSON file persisting chat_id -> sessionId").option("--workspaces-file <path>", "JSON file with [{name,path}] workspace list for the /workspace picker").option("--bots-file <path>", "JSON file with [{name,appId,appSecret,domain}] bot list for the /bot picker").option("--config-port <port>", "local config page port (default 8081)", "8081").option("--bot-open-id <id>", "bot open_id (for @mention gating)").option("--bot-user-id <id>", "bot user_id (for @mention gating)").option("--bot-name <name>", "bot display name (for @mention gating)").option("--allowed-users <ids>", "comma-separated open_id allowlist").option("--allow-all-users", "allow any Feishu user (dev only)").option("--group-policy <policy>", "group policy: open | allowlist | disabled", "allowlist").option("--require-mention <bool>", "groups must @mention the bot (true/false)", "true").option("--allow-bots <mode>", "accept peer bot messages: none | mentions | all", "none").option("--admins <ids>", "comma-separated admin open_ids (bypass group allowlist)").option("--workspace-manage <mode>", "who can add/edit/delete workspaces in Feishu: admin | all", "admin").option("--group-rules <json>", "JSON array of per-group rules [{chatId,policy,allowlist,blacklist}]").option("--allowed-tools <names>", "comma-separated tool whitelist (empty = all tools)").option("--dangerous-commands <patterns>", "comma-separated regex patterns; matching shell commands require card confirmation").option("--verification-token <token>", "webhook verification token").option("--encrypt-key <key>", "webhook signature encrypt key").option("--no-cards", "disable interactive approval/question cards").option("--no-reactions", "disable processing reactions").option("--no-media", "disable inbound media download and outbound media upload").option("--project-context <text>", "project background injected into every agent's system prompt").option("--idle-timeout <ms>", "dispose agents idle longer than this (0 = never)", "1800000").option("--idle-check-interval <ms>", "how often to check for idle agents", "300000").option("--reply-timeout <ms>", "interrupt a turn that produces no reply within this (0 = never)", "300000").option("--max-reply-chars <n>", "truncate a single reply to this many chars", "20000").option("--no-error-notify", "do not push errors to Feishu chats").option("--code-agent-model <provider/model>", "model for the /code agent (default: the default model)").option("--code-agent-context <text>", "system prompt for the /code agent").option("--subscribe-events", "handle subscribed events (doc comments, meeting invites)").option("--text-batch-ms <ms>", "burst text debounce delay (ms)", "600").addHelpText("after", `
Examples:
