...
function apply(ctx, config) {
	...
	async function ensureAgent(chatId) {
		...
		const setup = (agentCtx) => {
			...
			// Startup project-context injection: a fixed background section the
			// agent always sees, so every chat shares the project's context.
			if (config.projectContext?.trim()) {
				const sp = agentCtx.get("systemPrompt");
				if (sp !== void 0) {
					sp.section({ name: "feishu:project-context", order: 100, text: config.projectContext.trim() });
				}
			}
	...
	// --- slash commands -----------------------------------------------------
	const COMMANDS = ["/workspace", "/bot", "/model", "/stop", "/help", "/status", "/debug", "/reset", "/export", "/restart", "/autostart"];
