...
import { assistantText, chunkText, computeSignature, extractMediaRefs, looksLikeMarkdown, makeRateLimiter, sanitizeFilename, stripMarkdown, timingSafeEqual, validateWorkspace } from "./utils.js";
...
const Config = z.object({
	...
	workspaceManage: z.enum(["admin", "all"]).default("admin"),
	...
	codeAgentModel: z.string().default(""),
	codeAgentContext: z.string().default(""),
	idleTimeoutMs: z.number().default(30 * 60 * 1000),
	idleCheckIntervalMs: z.number().default(5 * 60 * 1000),
	replyTimeoutMs: z.number().default(5 * 60 * 1000),
	maxReplyChars: z.number().default(20000),
	errorNotify: z.boolean().default(true),
	subscribeEvents: z.boolean().default(false),
...
class ChatAgent {
	...
	async sendImage(filePath) {
		...
		const res = await this.clientRef.current.im.image.create({
		...
		return await this.send("image", { image_key: imageKey });
	}
	...
	async sendFile(filePath) {
		...
		const fileType = ["opus", "mp4", "pdf", "doc", "xls", "ppt", "stream"].includes(ext) ? ext : "stream";
		const res = await this.clientRef.current.im.file.create({
			data: { file_type: fileType, file_name: name, file }
		});
		const fileKey = res?.data?.file_key ?? res?.file_key;
		if (!fileKey) throw new Error("file upload returned no file_key");
		return await this.send("file", { file_key: fileKey });
	}
	/** Choose post(md) when the text looks like markdown, else plain text. */
	async reply(text) {
		// Enforce a per-reply length cap before chunking.
		if (this.config.maxReplyChars > 0 && text.length > this.config.maxReplyChars) {
			text = text.slice(0, this.config.maxReplyChars) + "\n…[回复已截断]";
		}
		// Send any markdown media references (existing local files) as attachments.
		const { text: cleanText, media } = extractMediaRefs(text, (p) => existsSync(p));
		for (const m of media) {
			try {
				if (m.kind === "image") await this.sendImage(m.path);
				else await this.sendFile(m.path);
			} catch (e) {
				this.log("media send error", e);
			}
		}
		if (cleanText === "") return;
		const isMd = looksLikeMarkdown(cleanText);
		const max = isMd ? this.config.maxMdChunk : this.config.maxTextChunk;
		for (const part of chunkText(cleanText, max)) {
			if (isMd) {
				try {
...
function apply(ctx, config) {
	...
	/** Send an error message to a chat (in addition to the log). */
	function notifyError(chatId, err) {
		if (config.errorNotify === false) return;
		const holder = chats.get(chatId);
		if (!holder) return;
		const msg = `❌ 出错了：${err instanceof Error ? err.message : String(err)}`;
		holder.send("text", { text: msg }).catch(() => {});
	}
	/** Append an audit line (who used what command when). */
	function audit(chatId, openId, action, detail) {
		const who = openId || "unknown";
		log(`[audit] ${new Date().toISOString()} chat=${chatId} user=${who} action=${action}${detail ? " " + detail : ""}\n`);
	}
	/** chat_id -> ChatAgent */
	const chats = new Map();
	...
	/** chat_id -> { timer } for reply-timeout tracking. */
	const pendingTurn = new Map();
	/** Set of active reminder timers, cleared on shutdown. */
	...
	/** The open_id of the user who clicked a card button. */
	function cardOperatorOpenId(event) {
		return event?.operator?.operator_id?.open_id ?? event?.operator?.open_id ?? "";
	}
	/** Whether this user may add/edit/delete workspaces. */
	function canManageWorkspaces(openId) {
		if (config.workspaceManage === "all") return true;
	...
	/** Arm a per-chat reply timeout; fires if no assistant reply arrives in time. */
	function armReplyTimeout(chatId) {
		if (config.replyTimeoutMs <= 0) return;
		const prev = pendingTurn.get(chatId);
		if (prev) clearTimeout(prev.timer);
		const timer = setTimeout(() => {
			pendingTurn.delete(chatId);
			const holder = chats.get(chatId) ?? codeAgents.get(chatId);
			if (!holder) return;
			holder.agent.cancel({ kind: "user" });
			holder.send("text", { text: "⏱ 回复超时，已中断当前生成。" }).catch(() => {});
		}, config.replyTimeoutMs);
		pendingTurn.set(chatId, { timer });
	}
	function dispatchImmediate(chatId, text) {
		ensureAgent(chatId).then((holder) => {
			...
			armReplyTimeout(chatId);
		}).catch((e) => log(`[feishu] dispatch: ${e instanceof Error ? e.message : e}\n`));
	}

	// --- agent lifecycle ----------------------------------------------------
	/** Build the agent setup callback shared by normal and code agents. */
	function makeSetup(selection, contextName, contextText) {
		return (agentCtx) => {
			...
			// Fixed background section the agent always sees.
			if (contextText?.trim()) {
				const sp = agentCtx.get("systemPrompt");
				if (sp !== void 0) {
					sp.section({ name: contextName, order: 100, text: contextText.trim() });
				}
			}
	...
	async function ensureAgent(chatId) {
		...
		const setup = makeSetup(selection, "feishu:project-context", config.projectContext);
		...
		if (state.chats[chatId]) {
			...
			try {
				...
				// A corrupt/unreadable persisted session (e.g. a seq gap in the
				// JSONL log) must not wedge the chat. Drop the bad mapping and
				// fall back to a fresh session.
				log(`[feishu] resume failed for chat ${chatId}: ${e instanceof Error ? e.message : e}\n`);
	...
	/** chat_id -> code ChatAgent (for /code routing). */
	const codeAgents = new Map();
	/** session.id -> code ChatAgent, for reply routing. */
	const codeBySession = new Map();
	/** Create (or reuse) the code-specialized agent for a chat. */
	async function ensureCodeAgent(chatId) {
		let holder = codeAgents.get(chatId);
		if (holder) return holder;
		const selection = ctx.get("agentDefaultModel").currentSelection();
		const agents = ctx.get("agents");
		let provider = selection.provider;
		let model = selection.model;
		if (config.codeAgentModel?.trim()) {
			const slash = config.codeAgentModel.indexOf("/");
			if (slash > 0) { provider = config.codeAgentModel.slice(0, slash).trim(); model = config.codeAgentModel.slice(slash + 1).trim(); }
			else model = config.codeAgentModel.trim();
		}
		const codeSelection = { provider, model };
		const agentOptions = { provider, model };
		const setup = makeSetup(codeSelection, "feishu:code-context", config.codeAgentContext);
		const cwd = chatWorkspace(chatId);
		log(`[feishu] creating code agent for chat ${chatId} (cwd ${cwd}, model ${provider}/${model})\n`);
		const handle = await agents.create({
			sessionId: SessionId(`feishu-code-${chatId}-${randomUUID()}`),
			meta: { cwd },
			agentOptions,
			setup
		});
		holder = new ChatAgent(handle, chatId, clientRef, config);
		codeBySession.set(handle.agent.session.id, holder);
		codeAgents.set(chatId, holder);
		installInteractionProviders(holder);
		return holder;
	}
	...
	ctx.on("session/event", (session, event) => {
		...
		if (text === "") return;
		const holder = bySession.get(session.id) ?? codeBySession.get(session.id);
		if (holder === void 0) return;
		// A reply arrived — clear the pending reply-timeout for this chat.
		const pending = pendingTurn.get(holder.chatId);
		if (pending) { clearTimeout(pending.timer); pendingTurn.delete(holder.chatId); }
	...
	function sendWorkspacePicker(holder, chatId, openId) {
		...
		if (canManageWorkspaces(openId)) {
			elements.push({ tag: "action", actions: [{ tag: "button", text: { tag: "plain_text", content: "⚙️ 管理" }, type: "default", value: { k: "fs_ws_manage" } }] });
		}
		holder.send("interactive", {
			config: { wide_screen_mode: true },
	...
	/** Card listing workspaces with edit/delete buttons plus an add button. */
	function sendWorkspaceManageCard(holder, chatId) {
		const list = config.workspaces ?? [];
		const elements = [];
		if (!list.length) {
			elements.push({ tag: "div", text: { tag: "plain_text", content: "尚未配置任何工作区。" } });
		}
		list.forEach((ws, i) => {
			elements.push({ tag: "div", text: { tag: "lark_md", content: `**${ws.name}**\n\`${ws.path}\`` } });
			elements.push({ tag: "action", actions: [
				{ tag: "button", text: { tag: "plain_text", content: "✏️ 编辑" }, type: "default", value: { k: "fs_ws_edit", index: i } },
				{ tag: "button", text: { tag: "plain_text", content: "🗑 删除" }, type: "danger", value: { k: "fs_ws_del", index: i } }
			] });
		});
		elements.push({ tag: "action", actions: [
			{ tag: "button", text: { tag: "plain_text", content: "➕ 新增" }, type: "primary", value: { k: "fs_ws_add" } },
			{ tag: "button", text: { tag: "plain_text", content: "返回" }, type: "default", value: { k: "fs_ws_back" } }
		] });
		holder.send("interactive", {
			config: { wide_screen_mode: true },
			header: { template: "blue", title: { tag: "plain_text", content: "管理工作区" } },
			elements
		}).catch((e) => holder.log("workspace manage card error", e));
	}
	/** Card with two input fields (name, path) for add/edit. */
	function sendWorkspaceInputCard(holder, chatId, mode, index) {
		const ws = mode === "edit" ? config.workspaces?.[index] : null;
		holder.send("interactive", {
			config: { wide_screen_mode: true },
			header: { template: "blue", title: { tag: "plain_text", content: mode === "edit" ? "编辑工作区" : "新增工作区" } },
			elements: [
				{ tag: "div", text: { tag: "lark_md", content: "填写工作区信息：" } },
				{ tag: "input", name: "ws_name", placeholder: { tag: "plain_text", content: "工作区名字" }, value: ws?.name ?? "" },
				{ tag: "input", name: "ws_path", placeholder: { tag: "plain_text", content: "工作区路径" }, value: ws?.path ?? "" },
				{ tag: "action", actions: [
					{ tag: "button", text: { tag: "plain_text", content: "保存" }, type: "primary", value: { k: mode === "edit" ? "fs_ws_save_edit" : "fs_ws_save_add", index } },
					{ tag: "button", text: { tag: "plain_text", content: "取消" }, type: "default", value: { k: "fs_ws_cancel" } }
				] }
			]
		}).catch((e) => holder.log("workspace input card error", e));
	}
	/** Card asking to confirm deleting a workspace. */
	function sendWorkspaceDeleteConfirm(holder, chatId, index) {
		const ws = config.workspaces?.[index];
		if (!ws) return;
		holder.send("interactive", {
			config: { wide_screen_mode: true },
			header: { template: "red", title: { tag: "plain_text", content: "确认删除" } },
			elements: [
				{ tag: "div", text: { tag: "lark_md", content: `确认删除工作区 **${ws.name}**？\n\`${ws.path}\`` } },
				{ tag: "action", actions: [
					{ tag: "button", text: { tag: "plain_text", content: "确认删除" }, type: "danger", value: { k: "fs_ws_confirm_del", index } },
					{ tag: "button", text: { tag: "plain_text", content: "取消" }, type: "default", value: { k: "fs_ws_cancel" } }
				] }
			]
		}).catch((e) => holder.log("workspace delete confirm error", e));
	}
	...
	/** Show a card to pick which model to use (from the llm registry). */
	async function sendModelPicker(holder) {
		const llm = ctx.get("llm");
		const current = ctx.get("agentDefaultModel").currentSelection();
		const providers = llm ? llm.listProviders() : [];
		const actions = [];
		for (const p of providers) {
			...
			for (const m of models) {
				actions.push({
					tag: "button",
					text: { tag: "plain_text", content: `${p.name} / ${m.name || m.id}` },
					type: (p.id === current.provider && m.id === current.model) ? "primary" : "default",
					value: { k: "fs_model", provider: p.id, model: m.id }
				});
			}
		}
		if (!actions.length) {
			await holder.send("text", { text: "当前没有可用的模型列表。" });
			return;
		}
		await holder.send("interactive", {
			config: { wide_screen_mode: true },
			header: { template: "blue", title: { tag: "plain_text", content: "选择模型" } },
			elements: [
				{ tag: "div", text: { tag: "lark_md", content: `当前：\`${current.provider}/${current.model}\`\n点击下方按钮切换模型：` } },
				{ tag: "action", actions }
			]
		}).catch((e) => holder.log("model picker error", e));
	}
	/** Save the default model and re-create the chat's agent so it takes effect. */
	async function switchModel(chatId, provider, model) {
		await ctx.get("agentDefaultModel").saveSelection({ provider, model });
		const holder = chats.get(chatId);
		if (holder) {
		...
		await h.send("text", { text: `✅ 模型已切换：${provider}/${model}` });
	}
	/** Interrupt the current generation for a chat. */
	async function stopGeneration(holder) {
		holder.agent.cancel({ kind: "user" });
		await holder.send("text", { text: "⏹ 已发送停止指令。" });
	}
	/** Route a prompt to the code-specialized agent for a chat. */
	function routeToCodeAgent(chatId, prompt) {
		ensureCodeAgent(chatId).then((ch) => {
			ch.agent.followup(createUserMessage({
				content: [{ type: "text", text: prompt }],
				source: { kind: "user" }
			}));
			armReplyTimeout(chatId);
		}).catch((e) => { log(`[feishu] code agent: ${e instanceof Error ? e.message : e}\n`); notifyError(chatId, e); });
	}
	/** Parse a reminder duration like "10s", "5m", "1h", "2d" into milliseconds. */
	function parseDuration(s) {
		const m = /^(\d+)\s*(s|m|h|d)?$/i.exec(String(s).trim());
		if (!m) return null;
		const n = Number(m[1]);
		const unit = (m[2] || "m").toLowerCase();
		const mult = unit === "s" ? 1000 : unit === "h" ? 3600000 : unit === "d" ? 86400000 : 60000;
		return n * mult;
	}
	/** Schedule a reminder that pushes a message to the chat after a delay. */
	function scheduleReminder(holder, chatId, text) {
		const parts = text.split(/\s+/);
		const dur = parseDuration(parts[1]);
		const msg = parts.slice(2).join(" ").trim();
		if (!dur || !msg) {
			holder.send("text", { text: "用法：/remind <时长> <提醒内容>，如 /remind 10m 记得喝水" }).catch((e) => holder.log("remind usage error", e));
			return;
		}
		const timer = setTimeout(() => {
			reminderTimers.delete(timer);
			holder.send("text", { text: `⏰ 提醒：${msg}` }).catch((e) => holder.log("remind fire error", e));
		}, dur);
		reminderTimers.add(timer);
		holder.send("text", { text: `✅ 已设置提醒，${parts[1]} 后提醒。` }).catch((e) => holder.log("remind ack error", e));
	}
	...
	/** Add a workspace after validation; returns an error string or null. */
	function addWorkspace(chatId, name, path) {
		const res = validateWorkspace({ name, path, list: config.workspaces, exists: existsSync });
		if (!res.ok) return res.error;
		config.workspaces.push(res.value);
		saveWorkspacesFile();
		return null;
	}
	/** Edit a workspace after validation; returns an error string or null. */
	function editWorkspace(chatId, index, name, path) {
		const res = validateWorkspace({ name, path, list: config.workspaces, editingIndex: index, exists: existsSync });
		if (!res.ok) return res.error;
		config.workspaces[index] = res.value;
		saveWorkspacesFile();
		return null;
	}
	/** Delete a workspace; chats using it fall back to the default root. */
	function deleteWorkspace(chatId, index) {
		const ws = config.workspaces?.[index];
		if (!ws) return "工作区不存在";
		config.workspaces.splice(index, 1);
		for (const [cid, sel] of Object.entries(state.workspaces ?? {})) {
			if (sel && sel.path === ws.path) delete state.workspaces[cid];
		}
		persistState();
		saveWorkspacesFile();
		return null;
	...
	function handleCardAction(event) {
		...
		const holder = chats.get(chatId);
		const operatorId = cardOperatorOpenId(event);
		const manage = () => canManageWorkspaces(operatorId);
		const deny = () => holder?.send("text", { text: "⛔ 你没有权限管理工作区。" }).catch((e) => holder.log("workspace deny error", e));
		...
		} else if (value.k === "fs_ws_manage") {
			if (!manage()) deny(); else sendWorkspaceManageCard(holder, chatId);
		} else if (value.k === "fs_ws_add") {
			if (!manage()) deny(); else sendWorkspaceInputCard(holder, chatId, "add", -1);
		} else if (value.k === "fs_ws_edit") {
			if (!manage()) deny(); else sendWorkspaceInputCard(holder, chatId, "edit", value.index);
		} else if (value.k === "fs_ws_del") {
			if (!manage()) deny(); else sendWorkspaceDeleteConfirm(holder, chatId, value.index);
		} else if (value.k === "fs_ws_confirm_del") {
			if (!manage()) deny();
			else {
				const err = deleteWorkspace(chatId, value.index);
				holder?.send("text", { text: err ? `❌ ${err}` : "✅ 工作区已删除" }).catch((e) => holder.log("workspace delete ack error", e));
			}
		} else if (value.k === "fs_ws_save_add" || value.k === "fs_ws_save_edit") {
			if (!manage()) deny();
			else {
				const input = event?.action?.input_value ?? {};
				const err = value.k === "fs_ws_save_add"
					? addWorkspace(chatId, input.ws_name ?? "", input.ws_path ?? "")
					: editWorkspace(chatId, value.index, input.ws_name ?? "", input.ws_path ?? "");
				holder?.send("text", { text: err ? `❌ ${err}` : "✅ 工作区已保存" }).catch((e) => holder.log("workspace save ack error", e));
			}
		} else if (value.k === "fs_ws_back") {
			sendWorkspaceManageCard(holder, chatId);
		} else if (value.k === "fs_ws_cancel") {
			// no-op: dismiss the input/confirm card
		...
		} else if (value.k === "fs_model") {
			if (value.provider && value.model && chatId) {
				switchModel(chatId, value.provider, value.model).catch((e) => log(`[feishu] model switch: ${e instanceof Error ? e.message : e}\n`));
			}
		}
	}
	...
	// --- slash commands -----------------------------------------------------
	const COMMANDS = ["/workspace", "/bot", "/model", "/stop", "/code", "/remind", "/help", "/status", "/debug", "/reset", "/export", "/restart", "/autostart"];
	...
	/** Export the current conversation as a markdown file sent back to Feishu. */
	async function exportConversation(holder, chatId) {
		let messages;
		try {
			messages = holder.agent.session.deriveMessages();
		} catch (e) {
			await holder.send("text", { text: `❌ 导出失败：${e instanceof Error ? e.message : e}` });
			return;
		}
		const lines = [];
		...
		if (!lines.length) {
			await holder.send("text", { text: "当前会话还没有可导出的内容。" });
			return;
		}
		const md = `# 对话导出\n\n${lines.join("\n\n")}\n`;
		const dir = join(chatWorkspace(chatId), ".feishu-export");
		mkdirSync(dir, { recursive: true });
		const file = join(dir, `export-${Date.now()}.md`);
		writeFileSync(file, md, "utf8");
		try {
			await holder.sendFile(file);
		} finally {
	...
	function handleCommand(holder, chatId, cmd, text, openId) {
		audit(chatId, openId, cmd, text.slice(0, 80));
		switch (cmd) {
			case "/workspace": sendWorkspacePicker(holder, chatId, openId); return true;
			case "/bot": sendBotPicker(holder); return true;
			case "/model": sendModelPicker(holder).catch((e) => log(`[feishu] model picker: ${e instanceof Error ? e.message : e}\n`)); return true;
			case "/stop": stopGeneration(holder).catch((e) => log(`[feishu] stop: ${e instanceof Error ? e.message : e}\n`)); return true;
			case "/code": {
				const prompt = text.replace(/^\/code\s*/, "").trim();
				if (!prompt) {
					holder.send("text", { text: "用法：/code <你的代码任务描述>" }).catch((e) => holder.log("code usage error", e));
				} else {
					routeToCodeAgent(chatId, prompt);
				}
				return true;
			}
			case "/remind": scheduleReminder(holder, chatId, text); return true;
			...
			case "/export": exportConversation(holder, chatId).catch((e) => log(`[feishu] export: ${e instanceof Error ? e.message : e}\n`)); return true;
	...
	/** Download inbound image/file/audio/video resources to the chat workspace. */
	async function downloadInboundMedia(chatId, message, refs) {
		if (!config.enableMedia || !refs?.length) return "";
		const dir = join(chatWorkspace(chatId), ".feishu-media");
		mkdirSync(dir, { recursive: true });
		const notes = [];
		for (const ref of refs) {
			const fileKey = ref.fileKey;
			const messageId = message?.message_id;
			if (!fileKey || !messageId) continue;
			const type = ref.kind === "image" ? "image" : ref.kind === "audio" ? "audio" : ref.kind === "media" ? "video" : "file";
			const name = ref.name || `${fileKey}.${ref.kind === "image" ? "img" : "bin"}`;
			const dest = join(dir, sanitizeFilename(name));
			try {
				const res = await clientRef.current.im.messageResource.get({
					params: { type },
					path: { message_id: messageId, file_key: fileKey }
				});
				await res.writeFile(dest);
				notes.push(`[已保存${ref.kind === "image" ? "图片" : ref.kind === "file" ? "文件" : "媒体"}到 ${dest}]`);
			} catch (e) {
				log(`[feishu] media download: ${e instanceof Error ? e.message : e}\n`);
			}
		}
		return notes.join("\n");
	}
	function handleMessage(event) {
		...
		if (text.startsWith("/")) {
			...
			const openId = senderOpenId(event);
			ensureAgent(chatId).then((holder) => handleCommand(holder, chatId, cmd, text, openId))
				.catch((e) => { log(`[feishu] command: ${e instanceof Error ? e.message : e}\n`); notifyError(chatId, e); });
			return;
		}
		// Best-effort: add a processing reaction on the user's message.
		ensureAgent(chatId).then(async (holder) => {
			const reactionIdPromise = holder.addReaction(message.message_id);
			const mediaNote = await downloadInboundMedia(chatId, message, refs);
			dispatchText(chatId, mediaNote ? text + "\n" + mediaNote : text);
	...
	/** Summarize a subscribed (non-message) event for notification. */
	function summarizeEvent(type, event) {
		switch (type) {
			case "drive.file.comment.created_v1":
				return `文档评论：${event?.comment?.content ?? ""}`;
			case "calendar.calendar_event.created_v1":
				return `新日程：${event?.calendar_event?.summary ?? ""}`;
			default:
				return `收到事件 ${type}`;
		}
	}
	/** Handle an optional subscribed event (doc comments, meeting invites, …). */
	function handleSubscribedEvent(type, event) {
		const chatId = event?.chat_id ?? event?.open_chat_id;
		const holder = chatId ? (chats.get(chatId) ?? codeAgents.get(chatId)) : null;
		const summary = summarizeEvent(type, event);
		if (holder) {
			holder.send("text", { text: `📣 ${summary}` }).catch(() => {});
		} else {
			log(`[feishu] subscribed event ${type}: ${summary}\n`);
		}
	}
	...
	function handleIncoming(data) {
		...
		try {
			...
			else if (config.subscribeEvents && type) handleSubscribedEvent(type, data.event);
		} catch (e) {
	...
	/** Periodically dispose agents idle longer than `idleTimeoutMs` to free memory. */
	function startIdleCleanup() {
		if (config.idleTimeoutMs <= 0) return null;
		const timer = setInterval(() => {
			const now = Date.now();
			for (const [chatId, holder] of chats) {
				if (now - holder.lastActive <= config.idleTimeoutMs) continue;
				log(`[feishu] idle cleanup: disposing chat ${chatId}\n`);
				holder.handle.dispose().catch(() => {});
				chats.delete(chatId);
				bySession.delete(holder.agent.session.id);
				// Keep state.chats[chatId] so the conversation resumes on next message.
			}
	...
	// Local config page (bots + workspaces editor).
	startConfigServer();
	// Idle session cleanup (frees memory for long-running bots).
	const idleTimer = startIdleCleanup();
	// Graceful shutdown.
	function shutdown() {
		stopTransport();
